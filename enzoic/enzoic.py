...
class Enzoic:
    ...
    def __init__(self, api_key, api_secret, api_base_url='https://api.enzoic.com/v1'):
        ...
        self.auth_string = 'basic ' + base64.b64encode((api_key+':'+api_secret).encode('utf-8')).decode('utf-8')
    ...
    def check_password(self, password: str) -> bool:
        ...
        query_string = f'?partialmd5={md5[:10]}&partial_sha1={sha1[:10]}&sha256={sha256[:10]}'
        response = self._make_rest_call(self.api_base_url + self.PASSWORDS_API_PATH +
                                        query_string, 'GET', None)

        if response != '404':
            for candidate in response['candidates']:
                keys = candidate.keys()
                if ('md5' in keys and candidate['md5'] == md5) or ('sha1' in keys and candidate['sha1'] == sha1) or\
                        ('sha256' in keys and candidate['sha256'] == sha256):
                    return True
            return False
        else:
            return False
    ...
    def check_password_ex(self, password: str) -> Tuple[bool, bool, None, int]:
        ...
        if response != '404':
            for candidate in response['candidates']:
                ...
                if ('md5' in keys and candidate['md5'] == md5) or ('sha1' in keys and candidate['sha1'] == sha1) or \
                        ('sha256' in keys and candidate['sha256'] == sha256):
                    return True, candidate['revealedInExposure'], candidate['relativeExposureFrequency'],\
                           candidate['exposureCount']
            return False, revealed_in_exposure, relative_exposure_frequency, exposure_count
        else:
            return False, revealed_in_exposure, relative_exposure_frequency, exposure_count

    def check_credentials(self, username: str, password: str, last_check_date: datetime = None,
                          exclude_hash_types: list = None) -> bool:
        """
        ...
        if last_check_date is not None and last_check_date.isoformat() >= response['lastBreachDate']:
            return False
        ...
        for hash_spec in response["passwordHashesRequired"]:
            if exclude_hash_types is not None and hash_spec['hashType'] in exclude_hash_types:
            ...
            if hash_spec['hashType'] != PasswordType.BCrypt or bcrypt_count <= 2:
                ...
                credential_hash = Hashing._calc_credential_hash(
                    username=username, password=password, argon2_salt=response['salt'], hash_type=hash_spec['hashType'],
        ...
        if len(query_string) > 0:
            creds_response = self._make_rest_call(self.api_base_url + self.CREDENTIALS_API_PATH + query_string, 'GET',
                                                  None)

            if creds_response != '404':
                # loop through the candidate hashes returned and see if we have a match with the exact hash
                for candidate in creds_response['candidateHashes']:
    ...
    def get_exposures_for_user(self, username: str) -> dict:
        ...
        """
        response = self._make_rest_call(self.api_base_url + self.EXPOSURE_API_PATH + '?username=' + username, 'get',
                                        None)
    ...
    def get_exposure_details(self, exposure_id: str) -> Union[dict, None]:
        ...
        """
        response = self._make_rest_call(self.api_base_url + self.EXPOSURE_API_PATH + '?id=' + exposure_id, 'GET', None)
    ...
    def _make_rest_call(self, url: str, method: str, body: dict = None) -> Union[dict, str]:
        headers = {
        ...
        if r.status_code == 200 or r.status_code == 201:
            ...
            raise Exception('Unexpected error from Enzoic API: ' + str(r.status_code) + ' ' + r.text)
