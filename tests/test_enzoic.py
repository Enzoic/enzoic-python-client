...
class TestEnzoic:
    ...
    def test_constructor_with_empty_str_parameters(self, check_constructor_with_parameters):
...
class TestCheckCredentials:
    ...
    def test_ignored_hash_type_is_not_exposed(self, enzoic, password_types):
        assert enzoic.check_credentials('testpwdpng445', 'testpwdpng4452',
                                        exclude_hash_types=[password_types.VBulletinPost3_8_5]) is False

    def test_last_check_date_on_exposure(self, enzoic):
        assert enzoic.check_credentials('testpwdpng445', 'testpwdpng4452',
                                        last_check_date=datetime(2018, 3, 1)) is False
...
class TestGetExposureDetails:
    ...
    def test_valid_exposure_with_details(self, enzoic):
        ...
        assert {"Emails", "Passwords", "Usernames", "Website Activity"} == set(response["exposedData"])
...
class TestCheckPassword:
    ...
    def test_uncompromised_password(self, enzoic):
        assert enzoic.check_password('kjdlkjdlksjdlskjdlskjslkjdslkdjslkdjslkd') is False
    ...
    def test_uncompromised_password_no_exposure_stats(self, enzoic):
        compromised, revealed_in_exposure, relative_exposure_frequency, exposure_count = \
